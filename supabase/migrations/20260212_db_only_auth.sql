-- Migration: DB-only authentication (replaces GoTrue/Supabase Auth)
-- Password hashing via pgcrypto, server-side auth functions via SECURITY DEFINER.
-- Login/register now go through PostgREST RPC, not GoTrue. No more spinning.

-- 1. Enable pgcrypto for bcrypt password hashing
CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- 2. Add password_hash column to players table
ALTER TABLE public.players ADD COLUMN IF NOT EXISTS password_hash TEXT;

-- 3. authenticate_player(username, password)
--    Returns JSON: { success, player } or { error }
...
-- 4. register_player(campaign_code, username, password, display_name)
--    Returns JSON: { success, role, player } or { error }
...
BEGIN
  ...
  -- First registered user (with password) who has no existing password-GM becomes GM
  ...
  VALUES (actual_name, new_role, norm_user, crypt(p_password, gen_salt('bf')), true)
...
-- 5. set_player_password(player_id, new_password) -- GM utility
