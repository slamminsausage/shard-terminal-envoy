-- Ensure role-selection registration can create the first credentialed GM.
-- A seeded GM row may exist (e.g. GM-MASTER) without a password hash;
-- that row should not block first GM registration.
...
BEGIN
  ...
  -- Only consider credentialed GM accounts when enforcing single-GM signup.
  -- This excludes seeded GM rows that do not yet have a password hash.
  SELECT EXISTS (
    SELECT 1
    FROM public.players
    WHERE role = 'gm'
